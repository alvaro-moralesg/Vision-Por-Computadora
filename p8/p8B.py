...
i_flag = False
drawing = False 
mode = 0                #mode = 0 -> seleccion por arrastre ; mode = 1 -> seleccion 3 puntos ; mode = 2 -> seleccion 4 puntos
ix , iy = [], []
...
def pt_slct(event, x, y, flags, param):
    global img_o, img_d, img_slct, i_flag, ix, iy, mode
    if event == cv2.EVENT_LBUTTONDOWN:
        ix.append(x)
        iy.append(y)
        cv2.circle(img_d, (x,y), 5, (255, 0, 0), -1)
        if mode == 1 and len(ix) == 3:
            i_flag = True
        ...
        elif mode == 2 and len(ix) == 4:
            i_flag = True
            img_rect = tf.rect(img_d, ix, iy)
            cv2.imwrite('rect_figure.png',img_rect)
 
cv2.namedWindow('laven tana')
cv2.setMouseCallback('laven tana', slct_check)

print('Seleccion de imagen por arrastre. Presione "a" para cambiar al modo de insercion de imagen. Presione "h" para cambiar al modo de rectificacion.')
while (1): 
    ...
    if k == ord('q'):
        ...
        os.system('clear')
        if mode == 0:
            print('Modo de seleccion de imagen por arrastre.')
        elif mode == 1:
            print('Modo de insercion de imagen. Seleccione 3 puntos.')
        elif mode == 2:
            print('Modo de rectificaion de imagen. Seleccione 4 puntos.')
        ix = []
        iy = []
        i_flag = False
        img_d = img_o.copy()
    elif k == ord('a') or k == ord('h'):    #Cambio de modos
        if i_flag == True:
            print('Vuelva la imagen a su estado original pulsando "r"')
        else:
            #mode = not mode
            if k == ord('a') and mode != 1:
                mode = 1
                print('Cambio al modo de insercion de imagen. Seleccione 3 puntos.')
            elif k == ord('h') and mode != 2:
                mode = 2
                print('Cambio al modo de rectificacion. Seleccione 4 puntos.')
            else:
                mode = 0
